...
// テクスチャ座標の設定
//===================================================
void CObject3D::SetTexture(const float fX1, const float fY1, const float fX2, const float fY2)
{
	// 頂点情報のポインタ
	VERTEX_3D* pVtx;
	// 頂点バッファのロック
	...
	// テクスチャ座標の設定
	pVtx[0].tex = D3DXVECTOR2(fX1, fY1);
	pVtx[1].tex = D3DXVECTOR2(fX2, fY1);
	pVtx[2].tex = D3DXVECTOR2(fX1, fY2);
	pVtx[3].tex = D3DXVECTOR2(fX2, fY2);
...
// 生成処理
//===================================================
CObject3D* CObject3D::Create(const D3DXVECTOR3 pos, const D3DXVECTOR3 rot, const D3DXVECTOR3 size, const char* pTextureName, const bool bLight_off)
{
	...
	pObject3D->m_bLight_off = bLight_off;
	pObject3D->m_pos = pos;
	pObject3D->m_rot = rot;
	pObject3D->m_Size = size;
	pObject3D->Init();
	pObject3D->SetTextureID(pTextureName);
