...
void CMotion::Update(CModel** pModel,const int nNumModel)
{
	...
	if (m_nCount >= nFrame)
	{
		...
		// 範囲内にラップする
		m_nKey = math::Wrap(m_nKey, 0, m_aInfo[m_nType].nNumkey - 1);
	...
	if (m_nCounterMotionBlend >= nFrame && (m_bFinish == true || m_bFirst == true))
	{
		...
		// 範囲内にラップする
		m_nKeyBlend = math::Wrap(m_nKeyBlend, 0, m_aInfo[m_nTypeBlend].nNumkey - 1);
	...
	if (nowLine.find("MOTIONSET") != string::npos)
	{
		while (getline(File, line))
		{
			...
			if (line.find("END_KEY") != string::npos)
			{
				...
				nCntModel = math::Clamp(nCntModel, 0, pLoader->GetNumModel() - 1);
			}
