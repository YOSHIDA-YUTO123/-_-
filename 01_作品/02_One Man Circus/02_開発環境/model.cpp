...
void CModel::Draw(void)
{
	...
	// モデルクラスの取得
	CModelManager* pModel = CManager::GetModelManager();
...
void CModel::DrawMultTexture(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)modelInfo.dwNumMat; nCntMat++)
	{
		...
		{
			//テクスチャの設定
			pDevice->SetTexture(1, pTextureManager->GetAdress(m_nTextureMTIdx));
		}
		else
		{
			//テクスチャの設定
...
void CModel::SetTextureMT(const char* pTextureName)
{
	...
	// IDの取得
	m_nTextureMTIdx = pTextureManager->Register(pTextureName);
}
