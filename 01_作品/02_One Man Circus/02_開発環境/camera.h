...
public:
	...
	void SetCamera(const D3DXVECTOR3 posR);
	void SetZoom(const float fDestDistance, const int nFream);
	void MouseWheel(void);
	void MouseView(void);
	void PadView(void);
	void TrackingCamera(D3DXVECTOR3 posR, float fPalam = 1.0f);
	void Zoom(void);
	void Zoom(const float fDisDest,const float fZoomSpeed);
...
private:
	...
	int m_nZoomFream;			// ズームのスピード
	int m_nZoomCount;			// ズームフレームカウンタ
	float m_fDistance;			// 距離
	float m_fOldDistance;		// 前の距離保存
	float m_fDestDistance;		// 目的の距離
