...
unique_ptr<CShadow> CShadow::Create(const D3DXVECTOR3 pos, const float fWidth, const float fHeight, const D3DXCOLOR col)
{
	...
	// 取得できなかったら処理しない
	if (pTextureManager == nullptr) return nullptr;
	...
	// 影のテクスチャの設定
	pShadow->m_nTextureIdx = pTextureManager->Register("data/TEXTURE/effect/effect000.jpg");

	// 位置、向き、大きさの設定
	pShadow->m_pos = pos;
	pShadow->m_Size = { fWidth,0.0f,fHeight };
	pShadow->m_col = col;
	pShadow->Init();
