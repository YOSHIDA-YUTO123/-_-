...
void CRenderer::Draw(const int fps)
{
	...
	m_pD3DDevice->Clear(0,
		...
		D3DCOLOR_RGBA(100, 100, 100, 100), 1.0f, 0);
...
void CRenderer::Debug(const int nFps)
{
	...
	// 線の描画クラスの取得
	CLineRenderer *pLineRenderer = CLineRenderer::GetInstance();
	...
	{
		// 切り替え
		bShowLine = bShowLine ? false : true;
	}

	if (CManager::GetInputKeyboard()->GetTrigger(DIK_F3))
	{
		// 切り替え
		bShowDebug = bShowDebug ? false : true;
	}

	// 表示するかどうか
	CDebugProc::EnableShow(bShowDebug);

	// 切り替え
	pLineRenderer->SetShow(bShowLine);

	const char* pLineShow = bShowLine ? "ON" : "OFF";
	const char* pDebugShow = bShowDebug ? "ON" : "OFF";

	CDebugProc::Print("\n********************************************\n");
	CDebugProc::Print("              デバッグ情報\n");
	CDebugProc::Print("********************************************\n");
	CDebugProc::Print("デバッグ表示     : [ F3 ] (%s)\n", pDebugShow);
	...
	CDebugProc::Print("カメラの回転     : [ X = %.3f ],[ Y = %.3f ],[ Z = %.3f ]\n", pCamera->GetRotaition().x, pCamera->GetRotaition().y, pCamera->GetRotaition().z);
