...
// 最後の入力からのフレーム
//===============================================
int CInputKeyboard::GetLastPress(void)
{
	if (GetPressAnyKey())
	{
		// フレームカウンタリセット
...
// パッドのLスティック(X)処理
//===============================================
bool CInputJoypad::GetJoyStickLOutDeadZone(void)
{
	//ローカル変数宣言
	bool joykey = false;
...
// 最後の入力からのフレーム取得
//===============================================
int CInputJoypad::GetLastInput(void)
{
	if (GetPressAnyButton() || GetJoyStickL() || GetJoyStickR())
	{
		// フレームカウンタリセット
		m_nCntLastFream = 0;
	}
