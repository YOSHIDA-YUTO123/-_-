...
void CMeshField::Update(void)
{
	...
	{
		CMeshFieldWave::Config config;
		config.epicenter = Const::VEC3_NULL;
		config.fInRadius = 0.0f;
		config.fOutRadius = 100.0f;
		config.fHeight = 300.0f;
		config.fSpeed = 15.0f;
		config.fcoef = 0.1f;
		config.nTime = 120;
	...
	//	// 色を白に近づける
	//	col += (Const::WHITE - col) * COLOR_EASE;
...
bool CMeshField::Collision(const D3DXVECTOR3 pos,float *pOutHeight)
{
	...
	for (int nCnt = 0; nCnt < FIELD_INFO::NUM_POLYGON; nCnt++)
	{
		// 頂点のインデックス
...
void CMeshField::SetTextureID(const char* pTextureName)
{
	...
	// ファイルパス
	std::string filePath = "data/TEXTURE/field/";
