...
{
	const char* JSON_FILE = "data/FILE/texture_list.json"; // 読み込み用ファイル
...
int CTextureManager::Register(const char* pFilename)
{
	...
	if (nIdx == -1)
	{
		...
		if (FAILED(D3DXCreateTextureFromFile(pDevice,
			...
			&info.pTexture)))
		{
			//MessageBox(nullptr, "CTextureManager::Register() > テクスチャを読み込めませんでした", "Error", MB_OK);
			return -1;
		}
...
HRESULT CTextureManager::Load(void)
{
	...
	// ファイルを開く
	std::fstream file("data/TXT/texture.txt");
